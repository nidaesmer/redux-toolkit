  ...
  return (
    <div className="m-12 p-6 rounded relative">
      ...
      <div className="items-center justify-center mb-2">
        <input
          className="px-2 rounded py-1 rounded-r-none bg-purple-900 text-white ring-0 ring-offset-0 outline-none"
          type="text"
          value={todo}
          onChange={(e) => setTodo(e.target.value)}
        />
        <button
          className="ml-0.5 bg-purple-900 rounded-l-none text-white px-2 rounded text-sm py-0.5 "
      ...
      <div className="mt-2">
        <ul className="gap-y-1 grid items-center">
          {todos.map((todo, index) => (
            <li
              key={index}
              className="flex justify-between border-b pb-1 border-opacity-20 border-white
                        "
            >
              <button onClick={() => handleStatusChange(index)}>
                <span
                  className={`${todo.status ? "" : "line-through"} text-xs`}
                >
                  {todo.text}
                </span>
              </button>
              <button
                className="ml-2 bg-red-600 text-xs rounded px-3"
